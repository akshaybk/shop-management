...
router.get("/purchases/:shopId", async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Unable to fetch purchase history" });
  }
});
...
router.get("/:shopId", async (req, res) => {
  try {
    ...
    if (!await hasShopAccess(req.user, shopId)) return res.status(403).json({ success: false, message: "You do not have access to this shop" });
    ...
    return res.json({
      success: true,
      shop: { id: shop.id, name: shop.name, location: shop.location },
    ...
    return res.status(500).json({ success: false, message: "Unable to fetch inventory" });
  }
});

router.post("/purchase", authorizeRoles("SUPER_MANAGER", "MANAGER"), async (req, res) => {
  try {
    ...
    if (!shopId || !productId || !quantity) {
      return res.status(400).json({ success: false, message: "Valid shopId, productId, and positive quantity are required" });
    }
    if (!Number.isFinite(unitCost) || unitCost < 0) {
      return res.status(400).json({ success: false, message: "Unit cost must be a non-negative number" });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const purchase = await tx.stockPurchase.create({
        data: { shopId, productId, quantity, unitCost, totalCost, createdById: req.user.userId },
      });
    ...
    return res.status(500).json({ success: false, message: "Unable to purchase stock" });
  }
});
